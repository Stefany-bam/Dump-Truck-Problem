//A C program to simulate the Dump Truck problem 
...
void init()
{
	...
	for(i = 0; i < n1; i++)
		scanf("%d", &values.loading_time[i]);
	printf("Enter the weighing time values:\t");
	for(i = 0; i < n2; i++)
		scanf("%d", &values.weighing_time[i]);
	printf("Enter the travel time values:\t");
	for(i = 0; i < n3; i++)
		scanf("%d", &values.travel_time[i]);
	printf("\n");
}
...
void main()
{
	...
	while(clock <= 100 && fel_start != NULL)
	{
		if(fel_start->event == 0)
		{
			prev_clock = clock;
